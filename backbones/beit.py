...
class BEiTTransformerEncoderLayer(VisionTransformerEncoderLayer):
    ...
    def forward(self, x):
        # print("X Size check(1): ", x.shape)
        x = x + self.drop_path(self.gamma_1 * self.attn(self.norm1(x)))
        # print("X Size check(3): ", x.shape)
...
class BEiT(BaseModule):
    ...
    def resize_rel_pos_embed(self, checkpoint):
        ...
        # print("keys check(0): ", self.state_dict().keys())
        all_keys = list(state_dict.keys())
        # print("\n\n\nkeys check(1): ", all_keys)
        for key in all_keys:
            ...
            # if ('relative_position_bias_table' in key) and (key in self.state_dict().keys()):
    ...
    def forward(self, inputs):
        # print("Input Size check(4): ", inputs.shape)
        B = inputs.shape[0]
        ...
        for i, layer in enumerate(self.layers):
            # print("X Size check(4): ", x.shape)
            x = layer(x)
            # print("X Size check(5): ", x.shape)   # 4~5 안 나옴!!!
